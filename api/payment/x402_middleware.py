...
async def _verify_payment(payment_header: str, path: str) -> bool:
...
class X402Middleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if used < PAID_FREE_QUOTA:
            # Still within free quota — allow
            return await call_next(request)

        # Over quota — check for payment
        payment_header = request.headers.get("payment-signature")
        if payment_header:
            valid = await _verify_payment(payment_header, path)
            if valid:
                return await call_next(request)
            return JSONResponse(
                status_code=402,
